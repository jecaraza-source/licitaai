...
|---|---|---|---|---|---|
| A1 | **Proyecto Supabase de staging** + secrets/vars en GitHub + Vercel + branch protection + Environment `production` con reviewers | todo despliegue; los workflows ya referencian estos nombres | Dueño de la cuenta Supabase/Vercel | M | 🟡 **casi**: proyecto `licitaai-staging` (`vuoimnwhxzlfelacvinf`) creado + 59 migraciones + 12 functions + 6 secrets; GitHub `staging` env + secrets + branch protection en `main` + reviewer en `Production`. **Falta:** (1) desbloquear un falso positivo de secret-scanning en el push (clave local por defecto de Supabase en 39 archivos de test), (2) 8 env vars de Vercel para la rama `staging` (valores en `scratchpad/VERCEL-ENV-STAGING.txt`). Detalle: `runbooks/setup-staging.md` |
