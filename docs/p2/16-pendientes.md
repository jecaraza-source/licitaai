...
| A4 | Rotar las API keys de OpenAI y Anthropic compartidas en la validación local | seguridad | Ingeniería / dueño de las keys | S | ⏸️ **decisión (2026-08-28): las keys compartidas se quedan como están, no se rotan.** Riesgo aceptado por el dueño de las keys. Producción debe usar keys propias en `supabase secrets`, no las de la validación local |
...
|---|---|---|---|---|
| B1 | **C6 — respetar `max_concurrent_jobs` por organización en `reclamar_jobs`** | una org con muchos jobs acapara el worker pese a que la columna existe (R13) | ✅ **hecho** (`115fa9a`, migración `20260907000000`): `reclamar_jobs` respeta el cupo; un RUNNING con lease vencido se retoma exento. Test `p2-b1-b2-concurrencia` | M |
| B2 | **Fairness del worker**: selección round-robin por org en `reclamar_jobs` + métrica de latencia de arranque p95 **por org** en `/admin/salud` | mismo R13; hoy es FIFO global | ✅ **hecho** (mismo commit): round-robin por org + `jobs.arranque_seg_por_org` en `metricas_operacion()` | M |
| B3 | **C5 — `ai_cache`** + dedup de embeddings por hash de contenido | ahorro de costo directo; el flag `ai.cache` ya existe pero no tiene nada detrás | ✅ **hecho** (migración `20260911000000`, flag `ai.cache` OFF): tabla `ai_cache` global (clave `sha256(contenido):template:version:modelo`, solo `service_role`) + `ai_cache_buscar`/`ai_cache_guardar`; `document_chunks.contenido_sha256` + `embedding_por_hash()`. Cableado: dedup de embeddings en el job-handler `procesar-documento`, caché de análisis en `analizar-bases`. Tests: `p2-b3-ai-cache` (8). **Pendiente menor**: extender el caché a `auditar-*` / `estudio-mercado`; retención de entradas frías | L |
| B4 | **C4 — aplicar `modelos_permitidos` (allowlist) y `politica_modelo`** en la selección de modelo de los handlers y en `estimar_costo_ia` | control de costo por plan; hoy las columnas existen pero nada las lee | ✅ **hecho** (migración `20260909000000`, flag `ai.politica_modelo` OFF): `resolver_modelo_ia(org, deseado, confianza_baja)` aplica política + allowlist; `aplicar_plan_a_org` siembra por plan (**BASE→Haiku/economico** por decisión de negocio 2026-08-28; PRO→Haiku+Sonnet; ENTERPRISE→+Opus/avanzado_si_confianza_baja — propuesta de ing., ajustable sin migración). Helpers `resolverModelo()` (Node + Deno). **Las 9 Edge Functions de IA cableadas** (`deno check` limpio). Tests: `p2-b4-politica-modelo` (13). **Pendiente menor**: alinear la estimación de `crearJobConPresupuesto` al modelo resuelto (hoy sobre-estima con Sonnet → la conciliación C3 lo corrige a la baja, dirección segura) | M |
| B5 | **D5 — gate duro de aprobación humana** | hoy solo hay aviso visual (`<AvisoRevisionIA>`); el brief pide que la IA nunca marque cumplimiento de forma automática | ✅ **hecho** (migración `20260913000000`, flag `ai.gate_aprobacion` OFF): un solo gate en el punto irreversible — el paso a **`ENVIADA`**. `licitacion_analisis_ia_pendientes()` lista las versiones activas de `ai_results` en `PENDIENTE`; si hay alguna, `POST /estado` responde **409**. Solo un **ADMIN** puede omitir con `omitir_revision_ia: true` (queda en `audit_log` inmutable). `APROBADO` y `RECHAZADO` ambos desbloquean. La pestaña Liberación muestra el gate. Tests: `p2-b5-gate-aprobacion-ia` (8 integración + 5 e2e). Diseño: [`20-gate-aprobacion-ia.md`](20-gate-aprobacion-ia.md) | M |
| B6 | **D5b — endpoint de comparación de versiones de `ai_results`** (diff entre una versión y la que reemplaza) | revisión humana del historial | ✅ **hecho**: `GET /api/ai-results/[id]/diff` (`?contra=<otro_id>` opcional) + `src/lib/json-diff.ts` (diff estructural recursivo) + 12 tests | S |
...
| B9 | **R11 — higiene de feature flags**: issue de retiro + fecha por cada flag; sección de auditoría de flags en cada release; marcar los kill-switches | evitar ramas muertas y comportamiento sorpresa | ✅ **hecho**: inventario + tipo (Lanzamiento/Kill-switch) + fecha de retiro + dueño en [`19-higiene-flags.md`](19-higiene-flags.md); plantilla de auditoría para el CHANGELOG. Falta crear los issues de retiro (al habilitar el primer flag en prod) | S |
| B10 | **Referencias cruzadas rotas a `08-clasificacion-datos.md`** (el archivo real es `13-clasificacion-datos.md`) | consistencia de la documentación | ✅ **hecho**: corregidas en `02-arquitectura-objetivo`, `03-plan-incremental`, `06-riesgos-residuales`, `runbooks/fuga-de-datos` | S |
| B11 | **Auditoría de funciones `SECURITY DEFINER` expuestas por RPC** | endurecimiento; el fix `0df1bc1` ya cubrió las 21 del worker | ✅ **hecho** (migración `20260910000000`): `revoke execute ... from anon` en las 7 de gobierno de costo (`check_ai_budget`, `estimar_costo_ia`, `registrar_uso_ia`, `presupuesto_ia_disponible`, `ai_policy_de_org`, `reservar_presupuesto_ia`, `liberar_mi_reserva_ia`). Las llamables por `anon` del flujo de registro (`create_organization_for_signup`, `invitacion_info`) revisadas 1×1 y se dejan | M |
| B12 | **`set search_path` en 3 trigger functions** (`_audit_log_inmutable`, `_retencion_archive_inmutable`, `_deletion_requests_touch` — advisor `function_search_path_mutable`) y mover `pg_net` fuera de `public` | endurecimiento menor | advisor de staging | S | ✅ **hecho** (`set search_path = ''` en las 3, migración `20260908000000`; verificado que los triggers siguen disparando). `pg_net`: sus funciones ya viven en `net` y se llaman calificadas — reubicar la extensión es territorio de Supabase, se deja |
| B13 | **Test helpers no deben hardcodear la clave local de Supabase** | higiene; hoy hay que desbloquear el push a mano cada vez | ✅ **hecho**: `tests/helpers/local-supabase.mjs` resuelve `url` / `anonKey` / `serviceRoleKey` desde el entorno o `supabase status -o env`; los 41 archivos de test importan `LOCAL` en vez del literal `sb_secret_…`. **Nota**: el literal sigue en el historial de git (~30 commits) — el allowlist de `.gitleaks.toml` se conserva por eso; los pushes nuevos ya no lo tocan | M |
